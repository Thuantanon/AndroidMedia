...
uniform vec2 mEyeLeft;
uniform vec2 mEyeRight;
...
vec4 bigEye()
{
    ...
    float noPointDis = distance(mEyeLeft, vec2(0.0, 0.0));
    vec2 leftPoint = vec2(mEyeLeft.x, clamp(1.0 - mEyeLeft.y, 0.0, 1.0));
    vec2 rightPoint = vec2(mEyeRight.x, clamp(1.0 - mEyeRight.y, 0.0, 1.0));
