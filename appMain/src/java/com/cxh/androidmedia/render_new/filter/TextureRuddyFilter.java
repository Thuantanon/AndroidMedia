 ...
 * Desc :
 */
public class TextureRuddyFilter extends BaseGLBeautyFilter {
    ...
    public TextureRuddyFilter(int width, int height) {
        this(width, height, false);
    }

    public TextureRuddyFilter(int width, int height, boolean leftFace) {
        super(width, height);
        mLeftFace = leftFace;

        String vertexShader = FileUtil.readShaderFromAssets("filter/ruddy/vertex_shader.glsl");
        String fragShader = FileUtil.readShaderFromAssets("filter/ruddy/fragment_shader.glsl");
        ...
        mRuddyTexture = OpenGLUtils.loadTexture(AMApp.get(), R.drawable.face_blush);
...
//        GLES30.glVertexAttribPointer(mRuddyCoordLocation, 2, GLES30.GL_FLOAT, false, 0, BitsUtil.arraysToBuffer(texRuddyCoordsArray));
    ...
    public void setRuddy(float[] left, float[] right) {
        mRuddyLeft = left;
        mRuddyRight = right;
    }
}
