 ...
 * Desc :
 */
public class TextureMouthFilter extends BaseGLBeautyFilter {
    ...
    private float[] mMouthLeft;
    private float[] mMouthRight;

    public TextureMouthFilter(int width, int height) {
        super(width, height);

        String vertexShader = FileUtil.readShaderFromAssets("filter/mouth/vertex_shader.glsl");
        String fragShader = FileUtil.readShaderFromAssets("filter/mouth/fragment_shader.glsl");
    ...
    public int draw(int textureId, int width, int height) {
        ...
        OpenGLUtils.setUnifrom2fv(mProgramId, "mMouthLeft", mMouthLeft);
        OpenGLUtils.setUnifrom2fv(mProgramId, "mMouthRight", mMouthRight);
    ...
    public void setMouth(float[] left, float[] right) {
        mMouthLeft = left;
        mMouthRight = right;
    }
}
